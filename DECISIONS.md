...
- `/api/paises` (GET): `SELECT pais, AVG(score), COUNT(*) FROM scores GROUP BY pais HAVING COUNT(*) >= 10 ORDER BY AVG(score) DESC`. Piso de 10 jugadores por país (bajado de 100) para que el promedio sea algo razonable sin dejar la tabla vacía en las primeras semanas, y como incentivo a compartir para que el propio país aparezca.
